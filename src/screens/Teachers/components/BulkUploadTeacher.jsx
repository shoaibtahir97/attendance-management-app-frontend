...
import { useUploadBulkUsersMutation } from '../../../redux/slices/apiSlices/usersApiSlice';

const BulkUploadStudent = (props) => {
  const { open, handleClose, fetchStudents } = props;
  const { openNotification } = useNotification();
  const [fileName, setFileName] = useState('');
  const [uploadBulkUsers, { isLoading: loadingUploadBulkUsers }] =
    useUploadBulkUsersMutation();

  const CSVFileSchema = Yup.object().shape({
    teachersFile: Yup.mixed().required(),
  });
  ...
  const uploadUserFile = async (data) => {
    await uploadBulkUsers(data.usersFile)
      .unwrap()
      .then((res) => {
  ...
  return (
    <Dialog fullWidth={true} maxWidth="md" open={open}>
      <DialogTitle id="scroll-dialog-title">
        <Box
          ...
          <Typography variant="h6" sx={{ textAlign: 'left', my: 1 }}>
            Upload excel for adding bulk teachers
          </Typography>
          <IconButton
            onClick={handleClose}
            sx={{
      ...
      <DialogContent>
        <FormProvider methods={methods} onSubmit={handleSubmit(uploadUserFile)}>
          <RHFUploadSingleFile
            name="studentsFile"
            onDrop={handleDrop}
            fileName={fileName}
          ...
          <Box
            ...
            <Box>
              ...
              <Button
                ...
                htmlType="submit"
                type="primary"
                loading={loadingUploadBulkUsers}
                size="large">
                Upload
              </Button>
              <Button
